...
const PROVIDERS = {
  ...
  cnj:      { enabled: true },
  cenprot:  { enabled: true }, // via Claude AI web search
...
export default async function handler(req, res) {
  ...
  if (PROVIDERS.cenprot.enabled) {
    try {
      const razao = report.dadosCadastrais?.razaoSocial || "";
      const prompt = `Pesquise protestos em cartório para o CNPJ ${cnpjFmt} (${razao}).

Busque em:
1. pesquisaprotesto.com.br
2. Sites de consulta de protestos
3. Qualquer fonte pública disponível

Retorne APENAS um JSON com este formato exato:
{
  "status": "limpo" | "protestado" | "nao_encontrado",
  "quantidade": número ou null,
  "valorTotal": número em reais ou null,
  "registros": [
    { "valor": número, "cartorio": "nome ou número", "cidade": "cidade", "uf": "UF", "vencimento": "DD/MM/AAAA" }
  ],
  "fonte": "nome da fonte consultada",
  "obs": "observação adicional se houver"
}

Se não encontrar dados concretos, retorne status "nao_encontrado". Não invente dados.`;
      ...
      const r = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          system: "Você é um assistente de consulta de crédito. Responda SEMPRE com JSON puro, sem texto antes ou depois, sem markdown.",
      ...
      if (r.ok) {
        ...
        const text = (textBlock?.text || "").replace(/```(?:json)?/gi, "").trim();
      ...
      report.protestos = {
        ...
        fonte: "CENPROT",
        linkManual: "https://pesquisaprotesto.com.br",
        obs: "Serviço temporariamente indisponível.",
      };
  ...
  //     const d = await r.json();
  //     report.restricaoFinanceira = {
  //       score: d.score?.valor, classificacao: d.score?.classificacao,
  //       mensagem: d.score?.descricao, totalDebitos: d.debitos?.quantidade,
  //       valorDebitos: d.debitos?.valor_total, fonte: "Netrin",
  //     };
  ...
  // ── 3. PROCESSOS JUDICIAIS — CNJ DataJud ─────────────────────────────────
  if (PROVIDERS.cnj.enabled) {
        ...
        const r = await fetch(`https://api-publica.datajud.cnj.jus.br/${t.index}/_search`, {
          ...
          body: JSON.stringify({
            query: { bool: { should: [
              ...
              { match_phrase: { "partes.documento": cnpjFmt } },
              ...(report.dadosCadastrais?.razaoSocial ? [{ match_phrase: { "partes.nome": report.dadosCadastrais.razaoSocial } }] : []),
            ]}},
            size: 20,
    ...
    const acoesEmpresa = unicos.filter(p =>
      p.partes?.some(pt =>
        pt.documento === raw ||
        pt.documento === cnpjFmt ||
        (razao && pt.nome?.toUpperCase().includes(razao))
      )
    );
  ...
  // ── 4. CHEQUES — slot para Banco Central ─────────────────────────────────
...
function calcularScore(r) {
  ...
  if (cap >= 1000000) fatores.push({ label: `Capital social ${(cap/1e6).toFixed(1)}M`, impacto: 0, positivo: true });
  ...
  if (procEmp === 0) fatores.push({ label: "Sem processos da empresa", impacto: 0, positivo: true });
